...
export async function buildMockData() {
  ...
  // Petty cash entries share the expenses collection (type: 'petty_cash').
  expenses.push({ id: 'EXP-petty-1', type: 'petty_cash', amount: 120, category: 'transport', description: 'Taxi for supplies pickup', paidBy: 'Sara Ahmed — سارة أحمد', date: dayStr(now - 4 * DAY) });
  expenses.push({ id: 'EXP-petty-2', type: 'petty_cash', amount: 80, category: 'maintenance', description: 'Light bulb replacement', paidBy: 'Omar Hassan — عمر حسن', date: dayStr(now - 2 * DAY) });
  expenses.push({ id: 'EXP-petty-3', type: 'petty_cash', amount: 50, category: 'supplies', description: 'Printer paper', paidBy: 'Sara Ahmed — سارة أحمد', date: dayStr(now - 1 * DAY) });
  ...
  /* ── Suppliers ── */
  const suppliers = [
    { id: 'SUP-1', name: 'FreshCo — فريش كو', phone: '+20 100 200 3000', email: 'sales@freshco.example', address: 'Obour Market, Cairo', notes: 'Vegetables & dairy' },
    { id: 'SUP-2', name: 'MetroSupply — مترو للتوريدات', phone: '+20 100 400 5000', email: 'orders@metrosupply.example', address: '6th October Industrial Zone', notes: 'Dry goods & packaging' },
    { id: 'SUP-3', name: 'FarmDirect — مزرعة مباشرة', phone: '+20 100 600 7000', email: 'hello@farmdirect.example', address: 'Beheira', notes: 'Meat & poultry' },
  ];

  /* ── Rents (one per branch location, mixed statuses) ── */
  const rents = [
    { id: 'RNT-1', locationId: locations[0].id, locationName: locations[0].area, landlord: 'Kamal Realty — كمال العقارية', amount: 25000, dueDate: dayStr(now + 10 * DAY), period: 'monthly', status: 'upcoming', paymentMethod: 'bank-transfer', notes: 'Main branch lease' },
    { id: 'RNT-2', locationId: locations[1].id, locationName: locations[1].area, landlord: 'Fawzia Holdings — فوزية', amount: 18000, dueDate: dayStr(now - 3 * DAY), period: 'monthly', status: 'overdue', paymentMethod: 'cash', notes: '' },
    { id: 'RNT-3', locationId: locations[2].id, locationName: locations[2].area, landlord: 'Sobhy Properties — صبحي', amount: 22000, dueDate: dayStr(now - 20 * DAY), paidDate: dayStr(now - 18 * DAY), period: 'monthly', status: 'paid', paymentMethod: 'card', notes: '' },
  ];

  /* ── Cash advances (reference active workers) ── */
  const activeWorkers = workers.filter((w) => w.status === 'active');
  const cashAdvances = [
    { id: 'CA-1', workerId: activeWorkers[1].id, workerName: activeWorkers[1].name, amount: 1000, date: dayStr(now - 6 * DAY), description: 'Advance on salary', status: 'pending', notes: '' },
    { id: 'CA-2', workerId: activeWorkers[2].id, workerName: activeWorkers[2].name, amount: 500, date: dayStr(now - 15 * DAY), description: 'Emergency loan', status: 'deducted', notes: 'Deducted from this month’s salary' },
  ];

  /* ── Complaints ── */
  const complaints = [
    { id: 'CMP-1', clientId: clients[0].id, clientName: clients[0].name, phone: clients[0].phoneNumbers[0], description: 'Order arrived cold', category: 'food', status: 'open', priority: 'high', date: dayStr(now - 2 * DAY) },
    { id: 'CMP-2', clientId: clients[3].id, clientName: clients[3].name, phone: clients[3].phoneNumbers[0], description: 'Delivery took over an hour', category: 'service', status: 'in-progress', priority: 'normal', date: dayStr(now - 5 * DAY) },
    { id: 'CMP-3', clientName: 'Walk-in Guest — زائر', phone: '', description: 'Table was not cleaned before seating', category: 'cleanliness', status: 'resolved', priority: 'low', date: dayStr(now - 9 * DAY), resolutionNotes: 'Apologized and re-cleaned the table' },
  ];
