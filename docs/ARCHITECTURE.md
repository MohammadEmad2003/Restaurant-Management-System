...
A Super Admin manages licenses via `superAdmin.js` routes: extend/reduce/renew, set-forever, revoke/suspend, adjust maximum devices or max concurrent cashier sessions, issue a fresh activation token, force-logout all cashiers, or reset/revoke individual devices. Offline Days, Validation Interval, and Session Timeout are intentionally not exposed as separate controls — see above.

**Activation tokens — hashed, time-limited, single-use.** `activation_tokens` (a dedicated table, `licenseService.issueActivationToken`/`redeemActivationToken`) replaces the old `activation_token_encrypted` design, which had none of these properties: a token was reversibly encrypted (readable back to plaintext by anyone with `ACTIVATION_TOKEN_ENCRYPTION_KEY`), never expired, and could reactivate the same license an unlimited number of times. Now: only a SHA-256 hash is ever stored (the plaintext is shown exactly once, at issuance, in the Super Admin UI — the same pattern already used for device secrets); each token expires after a configurable TTL (72h default); and redemption is a single atomic `UPDATE ... WHERE consumed_at IS NULL AND expires_at > now() RETURNING id` (`redeemActivationToken`), so two concurrent activation attempts with the same token can never both succeed, and there is no local-JSON fallback that could reach this code path on a real desktop install (see below).

**License Authority — activation is now structurally online-only.** This exact backend codebase runs in two roles (`config.isLicenseAuthority`, `backend/src/config/index.js`):
- **The Authority** — exactly one centrally-hosted, trusted instance (this server, `bellacucina.duckdns.org`, `IS_LICENSE_AUTHORITY=true` in its own `.env` only). It owns activation-token redemption, per-device hardware binding, and the offline-license signing key.
- **A packaged desktop install** — never sets that flag. `authService.activateRestaurantLicense` no longer calls `licenseService.activateLicense` directly in this mode; it proxies the whole call over real HTTPS to `LICENSE_AUTHORITY_URL` (`services/licenseAuthorityClient.js`, `POST /api/license/authority/activate`) with **no local fallback**. This is what makes "activation requires the internet" a structural fact rather than a soft `connectivity.check()` a user could defeat by editing `/etc/hosts`/local files — the only code path that can flip a license to `'active'` requires reaching that specific HTTPS host, and TLS certificate validation fails a spoofed redirect that a Postgres-connection-string hosts-file trick previously would not have.

**Hardware binding — exact match, Super Admin-approved reset only.** `hardwareBindingService.js` derives a per-device identity from four SMBIOS/hardware serials — motherboard, system UUID, primary disk, CPU — collected in the Electron **main** process (trusted; not the renderer) via PowerShell/WMI (`frontend/electron/main.js`'s `collectHardwareComponents`) and exposed to the frontend through `window.desktop.getHardwareComponents()`. Bound once per device at first contact (after login, once a real `deviceId` exists — a restaurant can run several devices under one license, `maximumDevices` defaults to 2, so binding is per-device, never a single restaurant-wide identity a second legitimate machine would collide with) and re-verified on every subsequent online heartbeat (`POST /auth/heartbeat` → `licenseAuthorityClient.revalidate` → `POST /api/license/authority/revalidate`). Per product decision, matching is **exact** — no k-of-n fuzzy tolerance: any component change (disk swap, motherboard replacement, a firmware update rewriting SMBIOS) flags the binding `pending_reset` and immediately suspends that restaurant's license locally (reusing the already-built suspended-license UI, no new frontend plumbing needed) rather than silently re-binding. The mismatch reports *which* component(s) changed (`changedComponents`, e.g. `["disk"]`) — never the raw serial — so a Super Admin can tell a routine repair from a possible clone before approving a reset (`POST /superadmin/restaurants/:id/devices/:deviceId/approve-hardware-reset`, Dashboard → Devices tab), which revokes the old device/binding; the restaurant then needs a freshly issued token to re-activate.

**Known limitation (unchanged from before this work, tracked in §17):** offline-license *signing* still happens locally on every install (both the Authority and every desktop install persist their own ECDSA P-256 keypair under `DATA_DIR` — `utils/offlineLicenseCrypto.js`) — moving that off the desktop entirely would also require moving day-to-day login/heartbeat off-device, which conflicts with this app's offline-first design (a cashier must be able to log in with zero internet). What changed here is narrower but structural: the desktop can no longer independently decide a license is *active*, redeem a token, or bind hardware — every one of those now requires reaching the Authority.
...
| `IS_LICENSE_AUTHORITY` | `false` | `true` on exactly ONE centrally-hosted instance — see §8's "License Authority" section. Never set on a packaged desktop build. |
| `LICENSE_AUTHORITY_URL` | — | Base URL (scheme+host, no trailing slash) of the Authority — only meaningful when `IS_LICENSE_AUTHORITY` is not `true` |
...
1. **Business-data DB credentials still ship inside the desktop installer.** The License Authority work (§8) closed activation/hardware-binding specifically — a packaged install can no longer independently decide a license is active, redeem a token, or bind hardware. It did **not** touch business-data sync: `DATABASE_URL` (a restricted-but-still-real Postgres credential) still ships in every installer's bundled `.env` for the existing offline-first `records`/`devices`/`sessions` mirroring, unchanged from before this work. Someone who extracts it still has direct read/write access to that data. Fully closing this would mean routing business-data sync through an HTTP API too (not just licensing), which is a substantially larger rewrite of `repositories/secureStore.js`/`repositories/index.js`/`sync/syncEngine.js` — deliberately out of scope for the licensing-focused pass that added §8. A narrower, high-value follow-up: a dedicated, least-privilege Postgres role for desktop installs specifically (no write access to `licenses`/`activation_tokens`/`hardware_bindings` — the Authority already never depends on the desktop's own copy of these tables for anything, so this would be pure defense-in-depth, not a functional change).
2. **Local-file/local-database tampering while genuinely and permanently offline is not fully preventable.** A user with filesystem access can still directly edit the local business-data JSON while the device has never reconnected. Verified to be exploitable only while the backend has never regained genuine Postgres reachability since the edit — the moment it does, reads prefer the authoritative remote copy again. Mitigated by the monthly online-validation deadline and the persisted offline-license signing key (§8).
3. **Consider consolidating the repeated inclusive-date-range filtering logic** (seen with slight variations across `orderService`, `cashierShiftService`, `financeService`, `analyticsService`, `reservationService`) into one shared utility, to prevent the class of one-off off-by-one bug already found and fixed once.
4. **In-memory rate-limit buckets are never evicted** — acceptable for a single-process deployment but an unbounded, slow memory growth over long uptimes; add an eviction/expiry pass.
5. **A background sweep that proactively terminates sessions the moment a license naturally expires** (rather than only rejecting the *next* request from that session) would improve UX slightly — currently a session technically still "looks" logged in client-side until its next API call, even though that call is guaranteed to be rejected. Not a security gap (§8's `requireActiveLicense` already blocks every actual request), purely a nice-to-have.
