...
import { Card, PageHeader, Spinner, Badge, Modal, Avatar, DataTable, LocationSelect } from '../components/ui.jsx';
  ...
  return (
    <div className="fade-in">
        ...
        {filtered.map((c) => (
          <div key={c.id} className="card card--hover" style={{ padding: 18, cursor: 'pointer' }} onClick={() => viewHistory(c)}>
            <div className="row between">
              <div className="row" style={{ gap: 12 }}>
                ...
                <div>
                  ...
                  <div className="muted row" style={{ fontSize: 12.5, gap: 4 }}><Phone size={12} /> <span className="ltr">{c.phoneNumbers?.[0] || '—'}</span></div>
                  {(c.governorate || c.area) && <div className="muted row" style={{ fontSize: 12, gap: 4, marginTop: 2 }}><MapPin size={12} /> {[c.area, c.governorate].filter(Boolean).join(' · ')}</div>}
                </div>
              </div>
            ...
            <div className="row between" style={{ marginTop: 14, paddingTop: 12, borderTop: '1px solid var(--border)' }}>
              <span className="muted" style={{ fontSize: 12.5 }}>{c.visitCount} {t('clients.visits', 'visits')}</span>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={t('clients.new', 'New Customer')}
        ...
        <div className="field"><label>{t('clients.deliveryAddress', 'Delivery Address')}</label><input className="input" value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} placeholder={t('clients.addressPlaceholder', 'Street, building, floor…')} /></div>
