        ...
        <Card title={t('dashboard.salesTrend')} sub={`${money(sales.monthly)} / 30d`} className="card--hover">
          <ResponsiveContainer width="100%" height={260}>
            <AreaChart data={sales.trend} margin={{ left: -16, right: 8, top: 8 }}>
              ...
              <Area type="monotone" dataKey="value" name={t('dashboard.sales')} stroke="#7c3aed" strokeWidth={2.5} fill="url(#g1)" />
            </AreaChart>
          </ResponsiveContainer>
        </Card>
        ...
        <Card title={t('dashboard.bestSellers')} className="card--hover">
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={(sales.bestSelling || []).map((p) => ({ ...p, label: shortName(p.name, lang) }))} margin={{ left: -16, right: 8 }}>
              ...
              <YAxis tick={{ fontSize: 11, fill: 'var(--muted)' }} />
              <Tooltip content={<ChartTooltip />} />
              <Bar dataKey="revenue" name={t('dashboard.revenue')} radius={[8, 8, 0, 0]} fill="#a855f7" />
            </BarChart>
          </ResponsiveContainer>
        </Card>
            ...
            <div key={c.id} className="row between" style={{ padding: '9px 0', borderBottom: '1px solid var(--border)' }}>
              <div className="row" style={{ gap: 11 }}>
                ...
                <div>
                  ...
                  <div className="muted" style={{ fontSize: 12 }}>{c.visits} {t('loyalty.visits')}</div>
                </div>
              </div>
        ...
        <Card title={t('dashboard.byLocation', 'Revenue by Location')} className="card--hover">
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={(locations || []).map((l) => ({ ...l, label: shortName(l.name, lang) }))} margin={{ left: -16, right: 8 }}>
              ...
              <YAxis tick={{ fontSize: 11, fill: 'var(--muted)' }} />
              <Tooltip content={<ChartTooltip />} />
              <Bar dataKey="revenue" name={t('dashboard.revenue')} radius={[8, 8, 0, 0]} fill="#06b6d4" />
            </BarChart>
          </ResponsiveContainer>
        </Card>
