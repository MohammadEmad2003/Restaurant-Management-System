...
import { Card, PageHeader, Spinner, Stat, DateField } from '../components/ui.jsx';
  ...
  return (
    <div className="fade-in">
      ...
      <div className="grid grid--stats" style={{ marginBottom: 18 }}>
        <Stat label={t('finance.revenue30', 'Revenue (30d)')} value={money(profit?.revenue || 0)} icon={DollarSign} color="linear-gradient(135deg,#10b981,#34d399)" />
        <Stat label={t('finance.expenses30', 'Expenses (30d)')} value={money(profit?.expenses || 0)} icon={TrendingDown} color="linear-gradient(135deg,#ef4444,#f87171)" />
      ...
      <div className="grid grid--2" style={{ marginBottom: 18 }}>
        <Card title={t('finance.cashFlow', `Cash Flow (${period})`)}>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={cashflow?.series || []} margin={{ left: -14 }}>
              ...
              <Legend />
              <Bar dataKey="inflow" name={t('finance.income', 'Income')} fill="#10b981" radius={[6, 6, 0, 0]} />
              <Bar dataKey="outflow" name={t('finance.expensesLabel', 'Expenses')} fill="#ef4444" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </Card>

        <Card title={t('finance.incomeTrend', `Income Trend (${period})`)}>
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={income.series} margin={{ left: -14 }}>
              ...
              <YAxis tick={{ fontSize: 11, fill: 'var(--muted)' }} />
              <Tooltip formatter={(v) => money(v)} contentStyle={{ borderRadius: 12, border: '1px solid var(--border)' }} />
              <Line type="monotone" dataKey="value" name={t('finance.income', 'Income')} stroke="#7c3aed" strokeWidth={2.6} dot={false} />
            </LineChart>
          </ResponsiveContainer>
        </Card>
      </div>
      ...
      <Card title={t('finance.byLocation', 'Profit & Orders by Location')} style={{ marginBottom: 18 }}>
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={locations || []} margin={{ left: -14 }}>
            ...
            <Legend />
            <Bar yAxisId="l" dataKey="revenue" name={t('finance.revenue', 'Revenue')} fill="#10b981" radius={[6, 6, 0, 0]} />
            <Bar yAxisId="l" dataKey="profit" name={t('finance.profit', 'Profit')} fill="#7c3aed" radius={[6, 6, 0, 0]} />
            <Bar yAxisId="r" dataKey="orders" name={t('finance.orders', 'Orders')} fill="#06b6d4" radius={[6, 6, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </Card>
