...
export default function Products() {
  ...
  const del = async (id) => {
    if (!(await confirm({ message: t('products.deleteConfirm', 'Delete this product?'), danger: true, confirmLabel: t('common.delete') }))) return;
    try { await api.delete(`/products/${id}`); notify(t('products.deleted', 'Product deleted')); refetch(); }
    catch (e) { notify(e.response?.data?.error || 'Failed', 'error'); }
  };

  const { page, pageSize, totalPages, totalItems, setPage, setPageSize, paginatedData: paginatedProducts } = usePaginated(data ?? [], 10);
